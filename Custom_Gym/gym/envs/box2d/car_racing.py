...
import random # for random sampling of good ones
...
# This will forbid predicting turns that has the opposite direction of the car
...
def default_reward_callback(env):
    ...
    # To allow changes of lane in intersections without losing points
...
class CarRacing(gym.Env, EzPickle):
    ...
    which is a proxy to control the complexity of map, the number of lanes (num_lanes),
    the number of obstacles on the track (num_obstacles) and the probability that an obstacle is a bonus (prop_good_obstacles).
    ...
    prop_good_obstacles(flt 0)       The probability of the obstacle to be good, meaning
                                     extra reward when touched 
    ...
    def __init__(self, **kwargs):
        ...
        self._current_nodes = {} # A dict of dicts, dict[id][lane]=direction it can be in more than one tile at the same time, e.g. intersections
    ...
    def _set_config(self, 
        ...
        # proportion of good obstacles in the track
        self.prop_good_obstacles = prop_good_obstacles if prop_good_obstacles >= 0 else 0
    ...
    def check_unvisited_tiles(self,reward,done):
        if self.info['visited'].sum() / self.info.shape[0] > 0.7:
            done = True
        return reward,done
    ...
    def check_obstacles_touched(self,obstacle_value=OBSTACLE_VALUE):
        ...
        obs_count = self.obstacle_contacts[obs_not_visited]['count_delay'] > 0
        touched_obstacles = self.obstacle_contacts[obs_not_visited][obs_count]['obstacle_id'].tolist()
        # touched_indices = list(np.where(self.obstacle_contacts[obs_not_visited]['count_delay'] > 0)[0])
        obstacle_rwd = 0
        if obs_count.sum() > 0:
            # obstacle_rwd = obstacle_value
            obstacle_rwd = [-obstacle_value if any(idx in self.good_obstacle_ids for idx in touched_obstacles) else obstacle_value][0]
        return obstacle_rwd
    ...
    def _create_obstacles(self):
        ...
        if len(possible_candidates) > 0:
            for _ in range(self.num_obstacles):
                ...
                if len(possible_candidates) == 0: break
        n = round(int(float(len(self.obstacle_tiles_ids))) * self.prop_good_obstacles)

        self.good_obstacle_ids = random.sample(self.obstacle_tiles_ids, n)


        # This creates the objects for the obstacles
        for count,idx in enumerate(self.obstacle_tiles_ids):
            ...
            t.color = [[1.0, 1.0, 0.0] if idx in self.good_obstacle_ids else [0.86,0.08,0.23]][0]
    ...
    def _generate_track(self):
        if self.load_tracks_from is not None:
            ...
            try:
                ...
                # Merge the list with the obstacle_contacts array
                for idx, value in enumerate(self.obstacle_tiles_ids):
                    self.obstacle_contacts[idx]['obstacle_id'] = value
    ...
    def _create_track(self):
        ...
        self.obstacle_contacts = np.zeros((len(self.obstacles_poly)),dtype=
                [('count',int),('count_delay',int),('visited',bool),('obstacle_id', int)])
    ...
    def render(self, mode='human'):
        ...
        if mode=="rgb_array" or mode=="state_pixels" or mode=="HD":
            ...
            if pkg_resources.get_distribution("pyglet").version=='1.5.11':
                arr = np.fromstring(image_data.get_data(), dtype=np.uint8, sep='') # use this when pyglet==1.5.11
            else:
                arr = np.fromstring(image_data.data, dtype=np.uint8, sep='') # use this when pyglet==1.3.2
